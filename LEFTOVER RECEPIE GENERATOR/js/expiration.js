...
import { getPantryIngredients, removePantryIngredient, updatePantryIngredient } from './ingredients.js';
...
/**
 * Reset dismissed notifications (e.g. if ingredient date changes, or for testing).
 */
 ...
 * E.g. boosts matches using expiring soon / expired ingredients, or shows them first.
 * 
 * @param {Array} matchedRecipes - Recipe matches from calculateRecipeMatches
...
export function prioritizeExpiringRecipes(matchedRecipes) {
    ...
    return [...matchedRecipes].sort((a, b) => {
        // Count how many expiring ingredients are used in recipe A vs B
        const aCount = a.matchedIngredients.filter(ing => {
            // strip parenthetical substitutions if any, e.g. "soy sauce (using tamari)"
        ...
        // Secondary: standard match percentage
        ...
        const renderDashboard = (filterVal = 'all') => {
            ...
            const createRow = (item) => {
                ...
                dateInput.addEventListener('change', (e) => {
                    ...
                    updatePantryIngredient(item.id, { expiryDate: newDate });
                    
                    // Dispatch notifications update events
                ...
                // Recommend recipes button (chef hat style action)
                ...
                if (item.expiryDate) {
                    ...
                    clearBtn.addEventListener('click', () => {
                        updatePantryIngredient(item.id, { expiryDate: '' });
                        document.dispatchEvent(new CustomEvent('pantryUpdated'));
                        renderDashboard(filterSelect.value);
                    });
