...
import { loadPantryFromServer, getPantryIngredients, addPantryIngredient, removePantryIngredient, clearPantry, MAX_PANTRY_LIMIT } from './ingredients.js';
import { calculateRecipeMatches } from './recipeMatching.js';
import { loadExclusionsFromServer, getExcludedIngredients, addExcludedIngredient, removeExcludedIngredient, applyFilters, isExcluded } from './filters.js';
import { getActiveNotifications, prioritizeExpiringRecipes, getExpirationStatus } from './expiration.js';
import { loadFavoritesFromServer, toggleFavorite, isFavorite } from './favorites.js';
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Load state from backend first
    const user = getCurrentUser();
    if (user) {
        await Promise.all([
            loadPantryFromServer(),
            loadExclusionsFromServer(),
            loadFavoritesFromServer()
        ]);
        
        // Initialize user dietary/allergen checkbox defaults saved in profile
    ...
    const refreshRecipes = async () => {
        ...
        matches.forEach(({ recipe, matchPercentage, matchedIngredients, missingIngredients }) => {
            ...
            favBtn.addEventListener('click', async (e) => {
                e.stopPropagation();
                try {
                    const nowFav = await toggleFavorite(recipe.id, recipe);
        ...
        commonIngredients.forEach(item => {
            ...
            pill.addEventListener('click', async () => {
                ...
                if (existing) {
                    ...
                    try {
                        await addPantryIngredient(item.name, item.category, '');
                        document.dispatchEvent(new CustomEvent('pantryUpdated'));
                        renderPantry();
                        updateQuickAddSelectionState();
                        checkExpiringAlertBanner();
    ...
    pantryForm.addEventListener('submit', async (e) => {
        ...
        try {
            await addPantryIngredient(name, category, expiryDate);
    ...
    pantryClearAllBtn.addEventListener('click', async () => {
        if (confirm("Are you sure you want to clear all pantry ingredients?")) {
            try {
                ...
                alert("Error clearing pantry: " + err.message);
            }
        }
    });
    ...
    const renderExclusions = () => {
        ...
        exclusions.forEach(name => {
            ...
            removeBtn.addEventListener('click', async () => {
                try {
                    await removeExcludedIngredient(name);
                    renderExclusions();
                    refreshRecipes();
                } catch (err) {
                    alert("Error removing exclusion: " + err.message);
                }
            });
    ...
    excludeForm.addEventListener('submit', async (e) => {
        ...
        try {
            await addExcludedIngredient(name);
