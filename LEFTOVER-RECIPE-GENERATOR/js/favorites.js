import { getSessionToken, getCurrentUser } from './auth.js';
import { getRecipeById } from './recipes.js';

// Client-side cache to support synchronous getFavorites calls
let cachedFavorites = [];

/**
 * Gets all favorite recipe records for the current user from cache.
 * @returns {Array} List of favorites: [{ recipeId, note, dateAdded, recipeData }]
 */
...
/**
 * Loads the user's favorites from the server and updates the local cache.
 */
export async function loadFavoritesFromServer() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            console.error('Failed to load favorites from server:', res.statusText);
        }
    } catch (err) {
...
export function isFavorite(recipeId) {
    ...
    return cachedFavorites.some(fav => String(fav.recipeId) === String(id));
}

/**
 * Toggles a recipe's favorite status on the server and updates the cache.
 * @param {number|string} recipeId - The recipe ID
 ...
 */
export async function toggleFavorite(recipeId, recipeData = null) {
    const id = typeof recipeId === 'string' && recipeId.startsWith('external_') ? recipeId : parseInt(recipeId, 10);
    const token = getSessionToken();
    ...
    const res = await fetch('/api/favorites', {
        ...
        body: JSON.stringify({ recipeId: id, recipeData })
    });
    ...
    if (data.favorited) {
        cachedFavorites.push(data.favorite);
        return true;
    } else {
        cachedFavorites = cachedFavorites.filter(fav => String(fav.recipeId) !== String(id));
        return false;
    }
}

/**
 * Updates the custom note of a favorited recipe on the server and updates the cache.
 * @param {number|string} recipeId - The recipe ID
 * @param {string} note - User's custom review or instructions note
 */
export async function updateFavoriteNote(recipeId, note) {
    const id = typeof recipeId === 'string' && recipeId.startsWith('external_') ? recipeId : parseInt(recipeId, 10);
    const token = getSessionToken();

    const res = await fetch(`/api/favorites/${id}/note`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
    ...
    const idx = cachedFavorites.findIndex(fav => String(fav.recipeId) === String(id));
    if (idx > -1) {
        cachedFavorites[idx].note = note;
    }
    return cachedFavorites[idx];
}
...
export function getFavorite(recipeId) {
    const id = typeof recipeId === 'string' && recipeId.startsWith('external_') ? recipeId : parseInt(recipeId, 10);
    return cachedFavorites.find(fav => String(fav.recipeId) === String(id)) || null;
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const renderFavorites = (searchQuery = '') => {
        ...
        favRecipes.forEach(({ recipe, note }) => {
            ...
            if (recipe.thumbnail) {
                header.style.backgroundImage = `url(${recipe.thumbnail})`;
            ...
            textarea.addEventListener('input', () => {
                ...
                saveTimeout = setTimeout(async () => {
                    try {
                        await updateFavoriteNote(recipe.id, textarea.value);
                        noteStatus.textContent = 'Saved!';
                        setTimeout(() => {
                            if (noteStatus.textContent === 'Saved!') noteStatus.textContent = '';
                        }, 1500);
                    } catch (err) {
                        noteStatus.textContent = 'Save error';
                    }
                }, 800);
            });
